    ...
    switch (action) {
      case "store":
        if (googleToken === null && refresh_token === null) {
          ...
          return new Response(JSON.stringify({ success: true, message: "Tokens deleted" }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }

        // Store tokens
        const { error: storeError } = await supabase.from("google_auth_tokens").upsert({
          user_id: user.id,
          token: googleToken,
      ...
      case "retrieve":
        try {
          ...
          return new Response(JSON.stringify({ token: validToken }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        } catch (error) {
      ...
      case "check_token_status":
        try {
          const validToken = await getValidToken(user.id);
          return new Response(JSON.stringify({ status: "valid", token: validToken }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        } catch (error) {
          if (error.message === "REAUTH_REQUIRED") {
        ...
        try {
          ...
          if (!calendarResponse.ok) {
            if (calendarResponse.status === 401) {
              return new Response(JSON.stringify({ error: "ADMIN_REAUTH_REQUIRED" }), {
                status: 401,
                headers: { ...corsHeaders, "Content-Type": "application/json" },
              });
            }
        ...
        try {
          ...
          if (!calendarResponse.ok) {
            ...
            if (calendarResponse.status === 404) {
              return new Response(JSON.stringify({ success: true, message: "Event already deleted" }), {
                headers: { ...corsHeaders, "Content-Type": "application/json" },
              });
            }
            throw new Error(`Calendar API error: ${calendarResponse.statusText}`);
